@"
// js/ui/whisp-chat.ts – Zilion with full game state & card knowledge
...
const S = {
  av: null as HTMLElement | null, body: null as HTMLElement | null,
  eye: null as HTMLElement | null, pupil: null as HTMLElement | null,
  refl: null as HTMLElement | null, bubble: null as HTMLElement | null,
  chatM: null as HTMLDivElement | null, chatInp: null as HTMLInputElement | null,
  ...
  glowIntensity: 1, glowTarget: 1, glowDecay: 0, emotion: 0,
  sleeping: false, lastClick: 0,
...
function pickIdleTarget(): void {
  ...
  S.idleTargetX = pos.x; S.idleTargetY = pos.y; idleIdx++;
}
document.addEventListener('mousemove', (e: MouseEvent) => {
  S.lastMouseMove = Date.now();
  if (!S.av) return;
  const r = S.av.getBoundingClientRect();
  S.tX = Math.max(-EYERANGE, Math.min(EYERANGE, (e.clientX - r.left - r.width/2)*0.15));
  S.tY = Math.max(-EYERANGE, Math.min(EYERANGE, (e.clientY - r.top - r.height/2)*0.15));
});
...
function computeCircleCorner(): void {
  const circle = document.querySelector('.circle-wrapper') || document.getElementById('ritualCircle');
  if (circle) {
    const rect = circle.getBoundingClientRect();
    S.defaultLeft = rect.right - 65;
    S.defaultTop = rect.bottom - 65;
  } else {
    S.defaultLeft = window.innerWidth/2 + 200;
    S.defaultTop = window.innerHeight/2 + 200;
  }
  S.left = S.defaultLeft;
  S.top = S.defaultTop;
}
...
function createAvatar(): void {
  ...
  av.style.cssText = 'position:fixed; left:'+S.left+'px; top:'+S.top+'px; width:0; height:0; z-index:100000; cursor:grab; user-select:none; touch-action:none; opacity:0;';
  S.av = av;
  const body = document.createElement('div'); body.className = 'zelion-body';
  Object.assign(body.style, { position:'absolute', width:'100%', height:'100%', borderRadius:'50%', background:'radial-gradient(circle at 35% 35%, #d4e8a0, #5a8a3a 60%, #2a4a1a)', boxShadow:'inset 0 -8px 16px rgba(0,0,0,0.4), inset 0 4px 8px rgba(255,255,255,0.15), 0 0 20px #8ac060, 0 0 45px #4a7a2a' });
  S.body = body; av.appendChild(body);
  const eye = document.createElement('div');
  Object.assign(eye.style, { position:'absolute', top:'21px', left:'24px', width:'24px', height:'24px', background:'radial-gradient(circle, #f8fff8, #d4f0a0 90%)', borderRadius:'50%', overflow:'hidden', boxShadow:'inset 0 2px 4px rgba(0,0,0,0.2), 0 0 4px rgba(0,0,0,0.3)' });
  S.eye = eye;
  const pupil = document.createElement('div');
  Object.assign(pupil.style, { position:'absolute', top:'50%', left:'50%', width:'12px', height:'12px', margin:'-6px 0 0 -6px', background:'#1a3a0a', borderRadius:'50%' });
  S.pupil = pupil; eye.appendChild(pupil);
  const refl = document.createElement('div');
  Object.assign(refl.style, { position:'absolute', top:'50%', left:'50%', width:'4px', height:'4px', margin:'-8px 0 0 4px', background:'white', borderRadius:'50%', opacity:0.9 });
  ...
  setTimeout(() => {
    if (S.av) {
      S.av.style.width = '75px'; S.av.style.height = '75px';
...
function animateToCircleCornerAndGreet(): void {
  ...
  S.left = S.defaultLeft;
  S.top = S.defaultTop;
  S.av.style.left = S.left + 'px';
  S.av.style.top = S.top + 'px';
  setTimeout(() => {
    showCenterBubble("Oh wow, I thought you'd never wake up! You've been sitting there like a stone statue for hours! Just double‑click me if you need anything.");
...
function spawnEntranceSwirl(): void {
  ...
  const spawn = () => {
    ...
    const centerX = S.left + 37;
    const centerY = S.top + 37;
    const x = centerX + Math.cos(angle) * dist;
    const y = centerY + Math.sin(angle) * dist;
    spawnTrailParticle(x, y, 0.3);
...
function setupDrag(): void {
  ...
  const moveHandler = (e: MouseEvent) => {
    ...
    const dx = newLeft - S.left, dy = newTop - S.top;
    const dist = Math.sqrt(dx*dx+dy*dy);
    if (dist > 2) {
      const steps = Math.floor(dist / 2);
      const stepX = dx/steps, stepY = dy/steps;
      for (let i=1; i<=steps; i++) {
        const tx = S.left + i*stepX + 37, ty = S.top + i*stepY + 37;
        spawnTrailParticle(tx, ty, i/steps);
      }
    }
  ...
  const upHandler = (e: MouseEvent) => {
    ...
    S.left = Math.max(0, Math.min(innerWidth-75, S.left));
    S.top = Math.max(0, Math.min(innerHeight-75, S.top));
    av.style.left = S.left+'px'; av.style.top = S.top+'px';
...
function spawnTrailParticle(x: number, y: number, life: number = 0): void {
  ...
  const size = Math.max(4, 42 - life * 38);
  const opacity = Math.max(0.3, 1 - life * 0.7);
  Object.assign(p.style, {
    position:'fixed', left:(x-size/2)+'px', top:(y-size/2)+'px',
    width:size+'px', height:size+'px', borderRadius:'50%',
    background: Math.random()>0.5?'#ffd700':'#a0d07a',
    opacity:opacity.toString(), boxShadow:'0 0 '+ (6 + life*6) +'px currentColor',
    animation:'zelionTrailFade 0.8s ease-out forwards',
    pointerEvents:'none',
  });
  S.trailParts.appendChild(p); setTimeout(() => p.remove(), 800);
}
...
function applyIdleShift(): void {
  ...
  shiftPhase += 0.015;
  S.body.style.transform = `translate(${Math.sin(shiftPhase*1.3)*1.5}px, ${Math.cos(shiftPhase*1.7)*1.2}px) scale(${1+Math.sin(shiftPhase)*0.005})`;
}
...
function startRenderLoop(): void {
  const loop = () => {
    ...
    if (now > S.nextBlink) {
      ...
      S.blinkTarget = 0.05; setTimeout(()=>{S.blinkTarget=1;}, 120);
...
function buildGameStateSummary(): string {
  ...
  parts.push(`Suspicion (Kalgoth's Noose): ${kalgothsNoose.value}%`);
  parts.push(`Orbex Fragments: ${orbexFragments.value}/${maxOrbexFragments.value}, Corruption: ${corruptionLevel.value}%`);
  parts.push(`Circle Power: ${circlePower.value}%, Circle Mastery: ${circleMastery.value}`);
  ...
  parts.push(`Game Phase: ${currentPhase.value.status}`);

  // Resources
  parts.push(`Resources: Moss=${ingredients.value.nightshadeMoss}, Phlegm=${ingredients.value.cryptPhlegm}, Salts=${ingredients.value.bansheeSalts}, Ichor=${ingredients.value.demonIchor}, Bone=${ingredients.value.boneDust}`);
  ...
  // Equipped entities with stats
  const entityIds = equippedEntitySlots.value.filter(id => id);
  if (entityIds.length > 0) {
    parts.push(`Equipped Entities:`);
    entityIds.forEach((id, idx) => {
      const card = getCardById(id);
      const stats = getEntityCombatStats(id);
      if (card && stats) {
        parts.push(`  ${idx+1}. ${card.name} (${card.rarity} ${card.aspect}) – HP:${stats.hp} ATK:${stats.atk} DEF:${stats.def} RES:${stats.res} SPD:${stats.spd} CUN:${stats.cun} INIT:${stats.init} Loyalty:${stats.loyalty}`);
        if (card.abilities?.length) {
          card.abilities.forEach(a => parts.push(`    Ability: ${a.name} - ${a.effect}`));
        }
      }
    });
    // Combos
    ...
    if (combos.length > 0) {
      combos.forEach(c => {
        ...
        parts.push(`  Combo: ${names} → ${c.effect}`);
      });
    }
    const synergy = getAspectSynergyBonus();
    if (Object.keys(synergy).length > 0) {
      const s: string[] = [];
      if (synergy.hp) s.push(`HP+${synergy.hp}`);
      if (synergy.atk) s.push(`ATK+${synergy.atk}`);
      if (synergy.def) s.push(`DEF+${synergy.def}`);
      if (synergy.spd) s.push(`SPD+${synergy.spd}`);
      if (synergy.cun) s.push(`CUN+${synergy.cun}`);
      parts.push(`  Synergy: ${s.join(', ')}`);
    }
  }

  // Spells
  const spellIds = equippedSpellSlots.value.filter(id => id);
  if (spellIds.length > 0) {
    parts.push(`Equipped Spells:`);
    spellIds.forEach((id, idx) => {
      const card = getCardById(id);
      if (card) {
        const ss = card.stats as SpellStats;
        parts.push(`  ${idx+1}. ${card.name} – Cost:${ss.cost} Will, ${ss.effect}`);
      }
    });
  }

  // Owned card library summary (counts by rarity)
  const ownedCardIds = ownedCards.value.map(o => o.cardId);
  const ownedCardsByRarity: Record<string, string[]> = {};
  ownedCardIds.forEach(id => {
    const card = getCardById(id);
    if (card) {
      if (!ownedCardsByRarity[card.rarity]) ownedCardsByRarity[card.rarity] = [];
      ownedCardsByRarity[card.rarity].push(card.name);
    }
  });
  parts.push(`Card Library: ${ownedCardIds.length} unique cards`);
  for (const r of ['common','uncommon','rare','epic','legendary']) {
    if (ownedCardsByRarity[r]) {
      parts.push(`  ${r}: ${ownedCardsByRarity[r].join(', ')}`);
    }
  }
...
export function showBubble(text: string, isKalgoth=false): void { addOrQueue(isKalgoth?'kalgoth':'zelion', text); }
function startModalDodge(): void {
  setInterval(() => {
    ...
    if (any && !S.modalOpen) {
      S.dodgeLeft=S.left; S.dodgeTop=S.top; S.left=20; S.top=20;
...
function addStyles(): void {
  ...
  s.textContent = '@keyframes zelionParticle{0%{transform:translate(0,0)scale(0);opacity:1}100%{transform:translate(var(--tx),var(--ty))scale(1.2);opacity:0}}@keyframes zelionTrailFade{0%{opacity:0.9;transform:scale(1)}100%{opacity:0;transform:scale(0.1)}}@keyframes bubbleFadeIn{from{opacity:0;transform:translate(-50%,-70%)}to{opacity:1;transform:translate(-50%,-100%)}}';
  document.head.appendChild(s);
}
"@ | Set-Content -Path "D:\DARKNEST\js\ui\whisp-chat.ts"