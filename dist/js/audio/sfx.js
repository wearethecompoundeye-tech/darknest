...
export function initAudio() {
    ...
    Object.entries(sfxDefinitions).forEach(([key, def]) => {
        ...
        if (def.loop)
            loopingSounds[key] = sounds[key];
    });
    if (bgMusic)
        bgMusic.play();
}
export function playSfx(key, volumeMultiplier = 1.0) {
    if (!sfxEnabled || !sounds[key])
        return;
...
export function startLoop(key) {
    if (!sfxEnabled || !loopingSounds[key])
        return;
...
export function fadeInLoop(key, duration = 2000) {
    if (!sfxEnabled || !loopingSounds[key])
        return;
    const sound = loopingSounds[key];
    sound.volume(0);
...
export function fadeOutLoop(key, duration = 2000) {
    if (loopingSounds[key]) {
...
export function updateVolumes(master, sfx, music) {
    masterVolume = master;
    sfxVolume = sfx;
    musicVolume = music;
...
export function startCaveDrips() {
    if (dripInterval)
...
export function stopAllAudioProcesses() {
    stopAllLoops();
    stopAllAmbience();
    stopCaveDrips();
}
