...
import { state, updateState, autoSave, loadSaveData, CONSTANTS, circleQuality, tutorial, playerName, lastForageTime, health, will, maxWill, orbexFragments, orbexBoons, maxWill as maxWillSignal, mazePathsUnlocked, setAddLog, ownedCards, kalgothsNoose, ingredients, activeDemon, addCard, } from './state-signals.js';
import { setupUIEffects, updateUI, initOrbitAnimation, triggerScreenPulse } from '../ui/ui-renderer.js';
import { checkTutorialProgress, getContextualHint, showTutorialPopup, openWhispStats, setTutorialEnabled, isTutorialEnabled } from '../ui/tutorial.js';
...
import { summonEntity, collectAsh } from '../systems/summoning.js';
...
import { playSfx, startLoop, stopAllLoops, toggleSfx, toggleMusic, getSound } from '../audio/sfx.js';
import { updateAmbience } from '../audio/ambience-manager.js';
...
import { currentPhase, transition } from './gameReducer.js';
import { initLocale } from './localisation.js';
...
export class Game {
    ...
    constructor() {
        ...
        window.payTithe = payTithe;
        window.escapeGame = attemptEscape;
        window.openTome = openTome;
        window.openSatchel = openSatchel;
        window.openGrimoire = openGrimoire;
        window.saveGame = () => autoSave();
        ...
        window.talkToDemon = talkToDemon;
        window.showCaptureJar = openGrimoire;
    ...
    start() {
        ...
        effect(() => {
            ...
            if (el)
                el.textContent = `Status: ${currentPhase.value.status}`;
        });
    ...
    onReady() {
        ...
        const splashLogo = el('splashLogo');
        ...
        this.safeAttach("masterVolumeSlider", "input", e => this.masterVol = parseFloat(e.target.value));
        this.safeAttach("sfxVolumeSlider", "input", e => this.sfxVol = parseFloat(e.target.value));
        this.safeAttach("musicVolumeSlider", "input", e => this.musicVol = parseFloat(e.target.value));
        ...
        if (toggleCrafting && craftingContent)
            toggleCrafting.addEventListener("click", () => { craftingContent.classList.toggle("collapsed"); toggleCrafting.classList.toggle("collapsed"); });
        const toggleRitual = el("toggleRitualBtn");
        const ritualContent = el("ritualContent");
        if (toggleRitual && ritualContent)
            toggleRitual.addEventListener("click", () => { ritualContent.classList.toggle("collapsed"); toggleRitual.classList.toggle("collapsed"); });
        const toggleStats = el("toggleStatsBtn");
        const statsContent = el("playerStatsContent");
        if (toggleStats && statsContent)
            toggleStats.addEventListener("click", () => { statsContent.classList.toggle("collapsed"); toggleStats.classList.toggle("collapsed"); });
        const toggleBandolier = el("toggleBandolierBtn");
        const bandolierContent = el("bandolierContent");
        if (toggleBandolier && bandolierContent)
            toggleBandolier.addEventListener("click", () => { bandolierContent.classList.toggle("collapsed"); toggleBandolier.classList.toggle("collapsed"); });
        this.setupModalClosers();
        const playerNameDisplay = el("playerNameDisplay");
        if (playerNameDisplay)
            playerNameDisplay.innerText = playerName.value;
    ...
    updateSettingsUI() {
        ...
        if (masterSlider)
            masterSlider.value = this.masterVol.toString();
        if (sfxSlider)
            sfxSlider.value = this.sfxVol.toString();
        if (musicSlider)
            musicSlider.value = this.musicVol.toString();
        const masterLabel = el("masterVolLabel");
        if (masterLabel)
            masterLabel.innerText = Math.round(this.masterVol * 100) + "%";
        const sfxLabel = el("sfxVolLabel");
        if (sfxLabel)
            sfxLabel.innerText = Math.round(this.sfxVol * 100) + "%";
        const musicLabel = el("musicVolLabel");
        if (musicLabel)
            musicLabel.innerText = Math.round(this.musicVol * 100) + "%";
        const muteSfxBtn = el("muteSfxBtn");
        if (muteSfxBtn)
            muteSfxBtn.textContent = this.sfxEnabled ? "🔊 SFX ON" : "🔇 SFX OFF";
        const muteMusicBtn = el("muteMusicBtn");
        if (muteMusicBtn)
            muteMusicBtn.textContent = this.musicEnabled ? "🎵 MUSIC ON" : "🔇 MUSIC OFF";
        const whispBtn = el("toggleWhispBtn");
        if (whispBtn)
            whispBtn.textContent = isWhispEnabled() ? "👁️ Whisp ON" : "👁️ Whisp OFF";
        const tutorialBtn = el("toggleTutorialBtn");
        if (tutorialBtn)
            tutorialBtn.textContent = isTutorialEnabled() ? "📖 Tutorial ON" : "📖 Tutorial OFF";
    }
    toggleSfxMute() { this.sfxEnabled = !this.sfxEnabled; toggleSfx(this.sfxEnabled); this.updateSettingsUI(); }
    toggleMusicMute() { this.musicEnabled = !this.musicEnabled; toggleMusic(this.musicEnabled); this.updateSettingsUI(); }
    toggleWhisp() { const enabled = !isWhispEnabled(); setWhispEnabled(enabled); this.updateSettingsUI(); }
    toggleTutorial() {
    ...
    startWillRegen() {
        if (this.willRegenInterval)
            clearInterval(this.willRegenInterval);
        ...
        if (fragmentCount > 0 && fragmentCount <= boons.length) {
            ...
            if (!orbexBoons.value.includes(newBoon.id)) {
                updateState(() => {
                    ...
                    if (newBoon.id === 'will')
                        maxWillSignal.value += 10;
                    if (newBoon.id === 'resistance')
                        mazePathsUnlocked.value = [...mazePathsUnlocked.value, 'Collapsed'];
                });
