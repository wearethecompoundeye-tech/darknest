...
import { circleQuality, circleIntegrity, circlePower, runeSlots, tutorial } from '../core/state-signals.js';
...
const RUNE_COUNT = 12;
export function initCircleTracing() {
    circleCanvas = el("circleCanvas");
    if (!circleCanvas)
        return;
...
function handleMouseMove(e) {
    if (!isActive || gameState !== 'tracing' || !isDragging)
        return;
    const point = getCanvasPoint(e);
...
function getCanvasTouchPoint(e) {
    const rect = circleCanvas.getBoundingClientRect();
    const scaleX = circleCanvas.width / rect.width;
    const scaleY = circleCanvas.height / rect.height;
...
function processTracePoint(point) {
    ...
    if (lastAngle !== null) {
        ...
        if (angleHistory.length > SMOOTHNESS_WINDOW)
            angleHistory.shift();
...
function completeTrace(success) {
    gameState = 'completed';
    isActive = false;
    isDragging = false;
    ...
    if (success && finalQuality >= 0.3) {
        ...
        hasTraced = true;
        gameBus.emit(GameEvents.CIRCLE_TRACED, { quality: finalQuality });
        for (let i = 0; i < 100; i++) {
        ...
        narrateEvent(`The survivor traces a ritual circle in emerald light. The Seed resonates.`)
            .then(n => { if (n)
            addLog(`🌿 ${n}`, false, 'orbex'); })
            .catch(() => { });
        const ritualCircle = el("ritualCircle");
        if (ritualCircle)
            ritualCircle.classList.add("traced");
    ...
    if (statusEl)
        statusEl.textContent = success ? "Ritual Complete!" : "Trace Broken";
    const progressFill = el("grindProgress");
    if (progressFill)
        progressFill.style.width = "0%";
}
function startCompletionAnimation(quality) {
    let phase = 0;
...
function clearTrace() {
    ...
    const statusEl = el("grindStatus");
    if (statusEl)
        statusEl.textContent = "Click 'Trace Circle' to begin";
}
export function resetCircleAfterSummon() {
    runeSlots.value = ["", "", ""];
    ...
    const ritualCircle = el("ritualCircle");
    if (ritualCircle)
        ritualCircle.classList.remove("traced");
