...
export function getProgressionRarityBonus() {
    const bonus = {};
...
export function selectWeightedCard(typeFilter, aspectFilter, excludeIds) {
    const bonus = getProgressionRarityBonus();
    const weightedPool = [];
    for (const card of allCards) {
        if (!typeFilter.includes(card.type))
            continue;
        if (aspectFilter && card.aspect !== aspectFilter && card.aspect !== 'All')
            continue;
...
// ========== CONTEXTUAL REWARDS ==========
export function getBarterReward(enemyCard) {
    const types = ['spell', 'enhancement'];
    const aspectFilter = enemyCard.aspect;
    const enemyRarityValue = {
        common: 0, uncommon: 1, rare: 2, epic: 3, legendary: 4
    };
    const minRarity = enemyCard.rarity === 'common' ? 'common' :
...
export function getEchoReward(pathType) {
    const aspectMap = {
        'Warded': 'Void',
        'Collapsed': 'Earth',
        'Echoing': 'Air',
...
export function getHollowReward(fragmentIndex) {
    const types = ['land'];
    let targetRarity;
    if (fragmentIndex >= 6)
        targetRarity = 'legendary';
    else if (fragmentIndex >= 5)
        targetRarity = 'epic';
...
export function getUndiscoveredCard(typeFilter, ownedCardIds) {
    const undiscovered = allCards.filter(c => typeFilter.includes(c.type) &&
...
export function validateStarterCards() {
