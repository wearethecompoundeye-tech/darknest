...
import { equippedEntitySlots, equippedLandSlots, ownedCards } from '../core/state-signals.js';
import { allCards, getCardById } from '../data/cards.js';
...
function parseComboBonus(effect) {
    const bonus = {};
    if (!effect)
        return bonus;
    if (effect.includes('+1 ATK'))
        bonus.atk = (bonus.atk || 0) + 1;
    if (effect.includes('+2 ATK'))
        bonus.atk = (bonus.atk || 0) + 2;
    if (effect.includes('+1 HP'))
        bonus.hp = (bonus.hp || 0) + 1;
    if (effect.includes('+2 HP'))
        bonus.hp = (bonus.hp || 0) + 2;
    if (effect.includes('+1 SPD'))
        bonus.spd = (bonus.spd || 0) + 1;
    if (effect.includes('+1 CUN'))
        bonus.cun = (bonus.cun || 0) + 1;
    if (effect.includes('+1 DEF'))
        bonus.def = (bonus.def || 0) + 1;
    if (effect.includes('+1 RES'))
        bonus.res = (bonus.res || 0) + 1;
    if (effect.includes('+1 INIT'))
        bonus.init = (bonus.init || 0) + 1;
    return bonus;
}
export function applyComboBonuses(baseStats, cardId) {
...
// ========== ENHANCEMENT SCALING ==========
export function getEnhancementBonus(enhancementLevel) {
    return {
...
export function getEnhancedStats(cardId) {
    const card = getCardById(cardId);
...
export function getAspectSynergyBonus() {
    const equipped = equippedEntitySlots.value.filter(id => id && id.length > 0);
    if (equipped.length === 0)
        return {};
    const aspects = equipped.map(id => getCardById(id)?.aspect).filter(a => a);
    const aspectCounts = {};
...
function getAspectPrimaryStat(aspect) {
    const map = {
        'Void': 'cun',
        'Fire': 'atk',
        'Earth': 'def',
...
function getAspectSecondaryStat(aspect) {
    const map = {
        'Void': 'init',
        'Fire': 'init',
        'Earth': 'hp',
...
// ========== LAND GENERATION ==========
export function processLandGeneration() {
    const generation = [];
...
// ========== CARD UPGRADE SYSTEM ==========
export function canUpgradeCard(cardId) {
    const owned = ownedCards.value.find(c => c.cardId === cardId);
    if (!owned)
        return false;
    const card = getCardById(cardId);
...
// ========== LOYALTY SYSTEM ==========
export function calculateLoyaltyDecay(entityId, expeditionTurns) {
...
export function checkLoyaltyFlee(entityId, currentLoyalty) {
