...
import { ingredients, crafted, masteryLevel, circleIntegrity, knownRunes, discoveries, tutorial, addMasteryXP } from '../core/state-signals.js';
...
export function craftPowder() {
  ...
  `;
    ...
    startLoop("grinding");
    const canvas = overlay.querySelector("#grindCanvas");
    const ctx = canvas.getContext("2d");
    const progressFill = overlay.querySelector("#grindProgress");
    const statusEl = overlay.querySelector("#grindStatus");
    const bonusEl = overlay.querySelector("#bonusCounter");
    let grinding = false;
    let grindAmount = 0;
    let bonusPowders = 0;
    let lastAngle = null;
    ...
    const ZONE_INNER = 50;
    const ZONE_OUTER = 90;
    function drawMortar(pestleX, pestleY, inZone) {
    ...
    function handleMouseMove(e) {
        ...
        if (grindAmount >= REQUIRED_ROTATION)
            finishGrinding(true);
    }
    function finishGrinding(success) {
        grinding = false;
        stopLoop("grinding");
    ...
    function handleMouseUp() {
        if (grinding && grindAmount < REQUIRED_ROTATION)
            finishGrinding(false);
        grinding = false;
    }
...
export function craftPotion(precise = false) {
...
export function quickCraftPowder() {
    if (ingredients.value.nightshadeMoss < 1 || ingredients.value.cryptPhlegm < 1) {
        addLog("Missing Moss or Phlegm.", true);
        return;
    }
...
export function quickCraftPhial() {
    craftPotion(false);
}
export function craftRestorative() {
