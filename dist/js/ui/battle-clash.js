...
export function setClashEnabled(enabled) { clashEnabled = enabled; }
export function isClashEnabled() { return clashEnabled; }
export class CardClashAnimation {
    overlay = null;
    resolved = false;
    constructor() { this.injectStyles(); }
    async play(playerCard, enemyCard, container) {
        if (!clashEnabled)
            return;
    ...
    animate(playerEl, enemyEl, onComplete) {
        // Start positions
    ...
    injectStyles() {
        if (document.getElementById('clash-styles'))
            return;
