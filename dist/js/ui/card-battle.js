...
import { will, maxWill, getEquippedCards, getActiveEntity, addMasteryXP, } from '../core/state-signals.js';
...
import { applyAbility, processStatusEffects, calculateDamage } from '../systems/ability-engine.js';
...
export async function openCardBattleModal(config) {
    currentConfig = config;
    const playerCard = config.playerCard || getActiveEntity();
    if (!playerCard)
        throw new Error('No player card');
    const enemyCard = config.enemyCard;
    // Build stats
    const playerStats = playerCard.stats;
    ...
    battleState = {
        ...
        playerAbilities: (playerCard.abilities || []),
        enemyAbilities: (enemyCard.abilities || []),
    ...
    gameBus.emit(GameEvents.BATTLE_CLASH_START, { playerCard, enemyCard });
    // ---- Play cinematic clash inside the modal ----
    if (isClashEnabled()) {
        const clash = new CardClashAnimation();
        const contentDiv = battleModal.querySelector('.battle-content');
        if (contentDiv) {
    ...
    if (battleState.turn === 'enemy')
        setTimeout(() => enemyTurn(playerCard, enemyCard), 600);
}
// ---------- MODAL HTML ----------
function createBattleModal(playerCard, enemyCard) {
...
// ---------- RENDER ----------
function renderBattleUI(playerCard, enemyCard) {
    if (!battleState)
        return;
    ...
    if (turnEl)
        turnEl.textContent = battleState.turn === 'player' ? 'Player' : 'Enemy';
}
// ---------- STAT DISPLAYS ----------
function updateStatDisplays() {
    ...
    const set = (id, val) => { const e = document.getElementById(id); if (e)
        e.textContent = val; };
    const pct = (id, p) => { const e = document.getElementById(id); if (e)
        e.style.width = `${p}%`; };
...
// ---------- ACTIONS ----------
function handleAttack(playerCard, enemyCard) {
    ...
    playSfx('card_play');
    triggerScreenPulse('#ff6a2a');
    checkEnd(playerCard, enemyCard);
    if (battleState.enemyHP > 0) {
        battleState.turn = 'enemy';
        renderBattleUI(playerCard, enemyCard);
        setTimeout(() => enemyTurn(playerCard, enemyCard), 800);
    }
    else
        renderBattleUI(playerCard, enemyCard);
}
function handleAbility(playerCard, enemyCard) {
    if (!battleState || battleState.turn !== 'player')
        return;
...
function enemyTurn(playerCard, enemyCard) {
    if (!battleState || !combatContext || !currentConfig)
        return;
    ...
    playSfx('health drop');
    triggerScreenPulse('#8a2a2a');
    checkEnd(playerCard, enemyCard);
    if (battleState.playerHP > 0)
        battleState.turn = 'player';
    renderBattleUI(playerCard, enemyCard);
}
function handleSpellCast(index, playerCard, enemyCard) {
    if (!battleState || battleState.turn !== 'player')
        return;
...
function handleFlee() {
    ...
    if (Math.random() < 0.4 + battleState.fleeAttempts * 0.2) {
        ...
        setTimeout(() => { if (currentConfig?.onFlee)
            currentConfig.onFlee(); closeBattleModal(); }, 1000);
    }
    else {
...
// ---------- END ----------
function checkEnd(playerCard, enemyCard) {
    if (!battleState || !currentConfig)
        return;
    if (battleState.enemyHP <= 0) {
        ...
        gameBus.emit(GameEvents.BATTLE_END, { result: 'victory', playerCard, enemyCard });
        ...
        gameBus.emit(GameEvents.BATTLE_END, { result: 'defeat', playerCard, enemyCard });
        setTimeout(() => { if (currentConfig.onDefeat)
            currentConfig.onDefeat(); closeBattleModal(); }, 1500);
    }
}
function showVictoryOptions(enemyCard) {
    if (!battleModal || !currentConfig)
        return;
  ...
  `;
    content.querySelector('#victoryBarter').addEventListener('click', () => { if (currentConfig.onVictory)
        currentConfig.onVictory('barter'); closeBattleModal(); });
    content.querySelector('#victoryDestroy').addEventListener('click', () => { if (currentConfig.onVictory)
        currentConfig.onVictory('destroy'); closeBattleModal(); });
    content.querySelector('#victoryEnsnare').addEventListener('click', () => { if (currentConfig.onVictory)
        currentConfig.onVictory('ensnare'); closeBattleModal(); });
}
