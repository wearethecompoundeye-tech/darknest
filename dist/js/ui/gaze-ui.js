...
import { will, maxWill, dailyConsumableSlots, crafted } from '../core/state-signals.js';
import { useConsumable, reinforceWard, forceStartGazeEvent } from '../systems/gaze-event.js';
let gazeOverlay = null;
let willBar = null;
let timerDisplay = null;
let drainRateDisplay = null;
let wardElements = [];
let consumableElements = [];
export function showGazeUI(phase) {
    if (gazeOverlay)
        gazeOverlay.remove();
    ...
    `;
        ...
        if (startBtn)
            startBtn.addEventListener('click', () => { forceStartGazeEvent(); });
        if (dismissBtn)
    ...
    `;
        ...
        for (let i = 0; i < 3; i++) {
            const ward = el(`gazeWard${i}`);
            if (ward) {
                ward.addEventListener('click', () => reinforceWard(i));
                wardElements[i] = ward;
            }
            const consumable = el(`gazeConsumable${i}`);
            if (consumable) {
                consumable.addEventListener('click', () => useConsumable(i));
                consumableElements[i] = consumable;
            }
        }
...
export function updateGazeUI(data) {
    if (!gazeOverlay)
        return;
    if (willBar)
        willBar.style.width = `${data.willPercent * 100}%`;
    const willText = el('gazeWillText');
    if (willText)
        willText.textContent = `${Math.floor(will.value)}/${maxWill.value}`;
    if (timerDisplay)
        timerDisplay.textContent = Math.ceil(data.timerSeconds).toString();
    if (drainRateDisplay)
        drainRateDisplay.textContent = `Drain: ${data.drainRate.toFixed(1)}/s`;
    for (let i = 0; i < 3; i++) {
        const integrityEl = el(`gazeWardIntegrity${i}`);
        if (integrityEl) {
            const integrity = data.wardIntegrities[i];
            integrityEl.textContent = `${Math.floor(integrity)}%`;
            integrityEl.style.color = integrity > 50 ? '#a0d07a' : (integrity > 20 ? '#d4af37' : '#ff6a6a');
        }
        const ward = wardElements[i];
        if (ward) {
            ward.style.boxShadow = data.wardIntegrities[i] > 50 ? '0 0 20px rgba(160,208,122,0.5)' : (data.wardIntegrities[i] > 20 ? '0 0 30px rgba(212,175,55,0.7)' : '0 0 40px rgba(255,0,0,0.8)');
            ward.style.borderColor = data.wardIntegrities[i] > 50 ? '#a0d07a' : (data.wardIntegrities[i] > 20 ? '#d4af37' : '#ff6a6a');
        }
    }
    updateConsumableDisplay();
}
function updateConsumableDisplay() {
    for (let i = 0; i < 3; i++) {
        ...
        const iconEl = el(`gazeConsumableIcon${i}`);
        const countEl = el(`gazeConsumableCount${i}`);
        if (!slotId) {
            if (iconEl)
                iconEl.textContent = '⬤';
            if (countEl)
        ...
        const consumable = consumableElements[i];
        if (consumable) {
            const count = slotId === 'restorativeDraught' ? crafted.value.restorativeDraught : slotId === 'powderOfWarding' ? crafted.value.powderOfWarding : slotId === 'phialOfSubjugation' ? crafted.value.phialOfSubjugation : 0;
            consumable.style.opacity = count > 0 ? '1' : '0.4';
            consumable.style.cursor = count > 0 ? 'pointer' : 'not-allowed';
        }
    }
}
export function hideGazeUI() { if (gazeOverlay) {
    gazeOverlay.remove();
    gazeOverlay = null;
} willBar = null; timerDisplay = null; drainRateDisplay = null; wardElements = []; consumableElements = []; }
