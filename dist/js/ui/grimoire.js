...
import { ownedCards, equippedEntitySlots, equippedSpellSlots, equippedEnhancementSlots, equippedLandSlots, maxEntitySlots, maxSpellSlots, maxEnhancementSlots, maxLandSlots, equipCard, getCardQuantity, getCardEnhancementLevel, mergeDuplicate, getEntityCombatStats } from '../core/state-signals.js';
...
export function openGrimoire() {
    ...
    playSfx('tomeOpen');
    // Focus trap
    modal.querySelector('.grimoire-body')?.focus();
}
// ---------- Modal structure ----------
function buildModal() {
...
function renderContent() {
    ...
    if (!grid)
        return;
    currentTypeFilter = document.getElementById('grimFilterType').value;
    currentRarityFilter = document.getElementById('grimFilterRarity').value;
    currentAspectFilter = document.getElementById('grimFilterAspect').value;
    let filtered = [];
    ownedCards.value.forEach(oc => {
        const card = getCardById(oc.cardId);
        if (!card)
            return;
        if (currentTypeFilter !== 'all' && card.type !== currentTypeFilter)
            return;
        if (currentRarityFilter !== 'all' && card.rarity !== currentRarityFilter)
            return;
        if (currentAspectFilter !== 'all' && card.aspect !== currentAspectFilter)
            return;
        filtered.push({ ...card, quantity: oc.quantity, enhancementLevel: oc.enhancementLevel });
    });
    document.getElementById('cardCount').textContent = `${filtered.length} cards`;
    grid.innerHTML = '';
    ...
    ['grimFilterType', 'grimFilterRarity', 'grimFilterAspect'].forEach(id => {
        ...
        if (el)
            el.onchange = () => { selectedCardId = null; renderContent(); };
    });
}
// ---------- Card Element ----------
function createCardElement(card) {
    ...
    if (card.type === 'entity') {
        const stats = getEntityCombatStats(card.id) || card.stats;
        ...
        slot.addEventListener('click', () => {
            ...
            if (selectedCardId) {
                ...
                if (card && card.type === type) {
                    ...
                    addLog(`Slot requires ${type} card.`, true);
            }
            else
                addLog('Select a card first.', true);
        });
    });
}
// ---------- Selected Card Panel ----------
function renderSelectedPanel() {
  ...
  `;
    if (mergeBtn)
        panel.querySelector('#mergeBtn').addEventListener('click', () => { if (mergeDuplicate(selectedCardId)) {
            playSfx('card_enhance');
            renderContent();
        } });
}
// ---------- Combo Panel ----------
function renderComboPanel() {
    ...
    if (Object.keys(synergy).length > 0) {
        ...
        if (synergy.hp)
            html += `<p style="font-size:0.7rem; margin:2px 0;">+${synergy.hp} HP</p>`;
        if (synergy.atk)
            html += `<p style="font-size:0.7rem; margin:2px 0;">+${synergy.atk} ATK</p>`;
        if (synergy.def)
            html += `<p style="font-size:0.7rem; margin:2px 0;">+${synergy.def} DEF</p>`;
        if (synergy.spd)
            html += `<p style="font-size:0.7rem; margin:2px 0;">+${synergy.spd} SPD</p>`;
        if (synergy.cun)
            html += `<p style="font-size:0.7rem; margin:2px 0;">+${synergy.cun} CUN</p>`;
        html += '</div>';
    }
