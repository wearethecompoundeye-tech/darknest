// js/ui/ledger.ts
import { state, ledgerEntries, totalSummons, releasedDemons, totalExplorations, discoveries } from '../core/state-signals.js';
import { askOllama } from '../ai/ai-engine.js';
export function addLedgerEntry(type, data) {
    const entry = {
        timestamp: Date.now(),
...
export async function renderLedger(containerId) {
    ...
    entries.forEach(entry => { html += formatLedgerEntry(entry); });
    html += `</div></div>`;
    ...
    container.querySelectorAll('.ledger-filter').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            filterLedgerEntries(btn.dataset.filter);
        });
    });
}
function formatLedgerEntry(entry) {
    ...
    switch (entry.type) {
        case 'summon':
            content = `Summoned ${entry.demonName} (${entry.trait})`;
            break;
        case 'release':
            content = `Released ${entry.demonName}. Max Will +${entry.bonus || 5}`;
            break;
        case 'destroy':
            content = `Destroyed ${entry.demonName}. Ichor: +${entry.ichor}, Bone: +${entry.bone}`;
            break;
        case 'capture':
            content = `Captured ${entry.demonName}`;
            break;
        case 'explore':
            content = `Explored maze. Found: ${entry.items || 'nothing'}`;
            break;
        case 'discovery':
            content = `Discovered ${entry.discoveryType}: ${entry.name}`;
            break;
        case 'lore':
            content = `Lore: "${entry.text}"`;
            break;
        case 'tithe':
            content = `Paid tithe: 5 Ichor`;
            break;
        case 'craft':
            content = `Crafted ${entry.item} x${entry.amount}`;
            break;
...
function filterLedgerEntries(filter) {
