...
import { tutorial, crafted, totalSummons, equippedEntitySlots, circleQuality, gazeSurvivalCount, dailyConsumableSlots, activeDemon } from '../core/state-signals.js';
import { addLog } from './log-manager.js';
// Tutorial state
let tutorialActive = false;
let currentStepIndex = 0;
let tutorialEnabled = true;
let whispElement = null;
let dialogueBubble = null;
let sparkleContainer = null;
let highlightElement = null;
let stepTimeout = null;
let isWaitingForAction = false;
let actionCheckInterval = null;
...
function createWhisp(center = false) {
    if (whispElement) {
        whispElement.remove();
    }
...
function showDialogue(text, showContinue = false) {
    if (dialogueBubble)
...
function positionElements(target, position = 'bottom') {
    if (!whispElement || !dialogueBubble)
        return;
...
function advanceTutorial() {
    ...
    if (!tutorialActive || !tutorialEnabled)
        return;
...
function executeStep(index) {
    ...
    setTimeout(() => { if (whispElement)
        whispElement.style.opacity = '1'; }, 10);
...
export function isTutorialEnabled() { return tutorialEnabled; }
export function showTutorialPopup(stepId) { }
export function getContextualHint() { return "Listen to Zelion. He knows what he's doing... mostly."; }
export function openWhispStats() { }
