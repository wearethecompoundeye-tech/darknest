...
export function initWhispChat() {
    ...
    startIdleWander();
    startModalDodge();
    gameBus.on(GameEvents.WHISP_MESSAGE, (p) => {
...
function addOrQueue(sender, text) {
    if (S.chatM) {
        addChatMsg(sender, text);
    }
    else {
        S.messageQueue.push({ sender, text });
    }
}
// ========== AVATAR ==========
...
function spawnTrailParticle(x, y, life = 0) {
    if (!S.trailParts)
        return;
...
function startRenderLoop() {
    const loop = () => {
        ...
        if (now - S.lastMouseMove > 30000 && !S.sleeping) {
            S.sleeping = true;
            S.blinkTarget = 0.35;
        }
        else if (now - S.lastMouseMove < 30000 && S.sleeping) {
            S.sleeping = false;
            S.blinkTarget = 1;
        }
        if (S.body) {
        ...
        applyIdleShift();
        if (S.eye)
            S.eye.style.transform = `translate(${S.cX}px, ${S.cY}px) scaleY(${S.blinkState})`;
        if (S.refl)
            S.refl.style.transform = `translate(${S.cX * 0.3}px, ${S.cY * 0.3}px)`;
        rafId = requestAnimationFrame(loop);
    };
    rafId = requestAnimationFrame(loop);
}
function spawnAmbientParticle() {
    const c = S.av?.querySelector('.zelion-parts');
    if (!c)
        return;
    const p = document.createElement('div');
...
// ========== CHAT MODAL ==========
function toggleChat() { S.chatM ? closeChat() : openChat(); }
function openChat() {
    if (S.chatM)
        return;
    ...
    S.chatM = md;
    S.chatMsgs = document.getElementById('zelionChatMsgs');
    S.chatInp = document.getElementById('zelionChatInp');
    ...
    S.chatInp.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && S.chatInp?.value.trim()) {
            const txt = S.chatInp.value.trim();
            S.chatInp.value = '';
...
function closeChat() { if (S.chatM) {
    S.chatM.remove();
    S.chatM = null;
    S.chatInp = null;
    S.chatMsgs = null;
} document.removeEventListener('keydown', escClose); }
function escClose(e) { if (e.key === 'Escape')
    closeChat(); }
function posChatModal() {
    if (!S.chatM || !S.av)
        return;
    const r = S.av.getBoundingClientRect();
...
function addChatMsg(sender, text) {
    ...
    S.chatMsgs.appendChild(d);
    S.chatMsgs.scrollTop = S.chatMsgs.scrollHeight;
}
async function handleChatSend(userText) {
    const corr = corruptionLevel.value;
    if (corr > 50 && Math.random() < (corr - 30) / 70) {
        addChatMsg('kalgoth', "I hear your thoughts.");
        return;
    }
    if (corr > 80 && Math.random() < 0.2) {
        addChatMsg('zelion', "Kalgoth's shadow...");
        return;
    }
    try {
        const mem = getZilionMemory();
        const prompt = mem.getPersonalityPrompt();
...
export function showBubble(text, isKalgoth = false) { addOrQueue(isKalgoth ? 'kalgoth' : 'zelion', text); }
// ========== MODAL DODGE ==========
function startModalDodge() {
    setInterval(() => {
        const modals = document.querySelectorAll('.modal');
        let any = false;
        modals.forEach(m => { const s = m.style.display; if (s !== 'none' && s !== '')
            any = true; });
        if (any && !S.modalOpen) {
            S.dodgeLeft = S.left;
            S.dodgeTop = S.top;
...
function addStyles() {
    if (document.getElementById('zelion-final-styles'))
        return;
