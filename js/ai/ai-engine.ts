...
export async function askOllama(messages: ChatMessage[], model: string = DEFAULT_MODEL): Promise<string> {
  try {
    ...
    if (!ollamaOffline) { ollamaOffline = true; console.warn('Ollama offline, switching to fallbacks.'); }
    ...
    if (content.includes('demon')) return fallbacks.demon[Math.floor(Math.random() * fallbacks.demon.length)];
    if (content.includes('Whisp')) return fallbacks.whisp[Math.floor(Math.random() * fallbacks.whisp.length)];
    if (content.includes('undercrypt')) return fallbacks.room[Math.floor(Math.random() * fallbacks.room.length)];
    return 'The darkness offers no reply...';
  }
}

export async function narrateEvent(eventDescription: string): Promise<string> {
  const messages = [
...
export async function getDemonDialogue(demonName: string, demonTrait: string, situation: string): Promise<string> {
...
export async function getWhispCommentary(context: string, stateSummary: string): Promise<string> {
  const messages = [
...
export async function getMazeRoomDescription(cellType: string, demonTrait: string): Promise<string> {
  const messages = [
...
export async function getAcolyteMemory(acolyteName: string): Promise<string> {
  const messages = [
...
export async function getOrbexVision(): Promise<string> {
  const messages = [
