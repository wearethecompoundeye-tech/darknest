...
export function startPhialBrewing(onComplete?: (success: boolean, completedRounds: number) => void): void {
  ...
  const runeElements = overlay.querySelectorAll('.cauldron-rune') as NodeListOf<HTMLElement>;
  const roundDisplay = overlay.querySelector('#brewRoundDisplay') as HTMLElement;
  const progressFill = overlay.querySelector('#brewProgress') as HTMLElement;
  const messageEl = overlay.querySelector('#brewMessage') as HTMLElement;
  const stopBtn = overlay.querySelector('#stopBrewBtn') as HTMLButtonElement;
  const cancelBtn = overlay.querySelector('#cancelBrewBtn') as HTMLButtonElement;
  ...
  function clearSequenceInterval(): void {
    if (sequenceInterval) {
  ...
  function highlightRune(rune: string, isActive: boolean): void {
  ...
  function playSequence(): void {
    ...
    sequenceInterval = window.setInterval(() => {
      ...
      if (idx > 0) highlightRune(sequence[idx - 1], false);
  ...
  function handleRuneClick(e: MouseEvent): void {
  ...
  function finishBrew(success: boolean, completedRounds: number): void {
    ...
    if (onComplete) onComplete(success, completedRounds);
  }

  function cancelBrew(): void {
    stopLoop("phialBoiling");
    clearSequenceInterval();
    gameActive = false;
    runeElements.forEach(el => el.removeEventListener('click', handleRuneClick));
