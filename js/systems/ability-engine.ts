...
import type { EntityAbility, EntityStats } from '../data/cards.js';
...
export interface ActiveEffect {
  name: string;
  duration: number;
  onTurnStart?: (ctx: CombatContext, isPlayer: boolean) => void;
  onTurnEnd?: (ctx: CombatContext, isPlayer: boolean) => void;
  onDamageDealt?: (ctx: CombatContext, damage: number, isPlayer: boolean) => number;
  onDamageTaken?: (ctx: CombatContext, damage: number, isPlayer: boolean) => number;
  onHeal?: (ctx: CombatContext, heal: number, isPlayer: boolean) => number;
  statModifiers?: Partial<EntityStats>;
}

export const abilityRegistry: Record<string, (ctx: CombatContext, isPlayer: boolean, ability: EntityAbility) => void> = {
  ...
  'Void Shift': (ctx, isPlayer, ability) => {
    if (isPlayer) {
      ...
      ctx.playerEffects.push({
        name: 'Void Shift',
        duration: 1,
        onTurnEnd: (c, player) => { if (player) c.playerAttack -= bonusAtk; }
      });
  ...
  'Drowning Gaze': (ctx, isPlayer, ability) => {
    if (isPlayer) {
      ...
      ctx.enemyEffects.push({
        name: 'Drowning Gaze',
        duration: 1,
        onTurnEnd: (c, player) => { if (!player) c.enemyAttack += 2; }
      });
...
export function applyTriggeredAbility(
  ctx: CombatContext,
  isPlayer: boolean,
  trigger: EntityAbility['trigger'],
...
export function processStatusEffects(ctx: CombatContext, isPlayer: boolean): void {
