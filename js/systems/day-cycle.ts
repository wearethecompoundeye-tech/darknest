...
import { GameEvents, type TithePaidPayload } from '../core/events.js';
import { addLog } from '../ui/log-manager.js';
import { playSfx } from '../audio/sfx.js';
import { el } from '../core/dom-helper.js';
import { processLandGeneration } from './card-progression.js';
import { applyWardNooseReduction } from './familiar-manager.js';
import { startGazeWarning, setGazeActive } from './gaze-event.js';
import type { Speaker } from '../ui/log-manager.js';
...
function checkModalVisibility(): void {
  const modals = document.querySelectorAll('.modal');
  const anyModalOpen = Array.from(modals).some(modal =>
    (modal as HTMLElement).style.display === 'flex' || (modal as HTMLElement).style.display === 'block'
  );
...
function startTimer(): void {
  timerInterval = window.setInterval(() => {
    ...
    const gazeActive = (window as any).__gazeActive;
...
export function payTithe(): void {
  ...
  if (ingredients.value.demonIchor >= CONSTANTS.DAILY_ICHOR_REQUIREMENT) {
    ...
    addLog(`Tithe paid. Kalgoth's Noose will loosen at day's end.`, false, 'player');
      gameBus.emit<TithePaidPayload>(GameEvents.TITHE_PAID, { ichorCost: CONSTANTS.DAILY_ICHOR_REQUIREMENT });
...
export function attemptEscape(): void {
...
export function stopDayCycle(): void {
  if (timerInterval) clearInterval(timerInterval);
  if (modalObserver) modalObserver.disconnect();
}

export function setGazeActive(active: boolean): void {
  (window as any).__gazeActive = active;
}