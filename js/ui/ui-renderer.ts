...
import { el, elRequired } from '../core/dom-helper.js';
...
function drawRuneTethers(): void {
  const tetherCanvas = getEl("tetherCanvas") as HTMLCanvasElement | null;
  if (!tetherCanvas) return;
  ...
  for (let i = 0; i < 3; i++) {
    if (runeSlots.value[i]) filledIndices.push(i);
  }
...
function renderConsoleRuneSlots(): void {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (runeSlots.value[i]) {
      ...
      const ctx = canvas.getContext("2d");
      if (ctx) ctx.clearRect(0, 0, 48, 48);
    }
...
function updateOrbexPanel(): void {
  ...
  let nextMilestone = '';
  if (fragmentCount === 0) nextMilestone = 'Seek the first Acolyte alcove.';
  else if (fragmentCount === 1) nextMilestone = 'One fragment reclaimed.';
  else if (fragmentCount === 2) nextMilestone = 'Two fragments.';
  else if (fragmentCount === 3) nextMilestone = 'Three fragments. Halfway there.';
  else if (fragmentCount === 4) nextMilestone = 'Four fragments.';
  else if (fragmentCount === 5) nextMilestone = 'Five fragments. One remains.';
  else nextMilestone = 'Orbex is whole. Confront Kalgoth.';
...
function updateResourceSummary(): void {
  ...
  for (const ing in ingredients.value) {
    const qty = ingredients.value[ing as keyof typeof ingredients.value];
    if (qty > 0) {
      html += `<span>${icons[ing] || "?"} ${qty}</span>`;
    }
  }
  if (html === "") html = "<span>No resources</span>";
...
export function updateEtchFeedback(coverage: number, smoothness: number): void {
  etchCoverage = coverage;
  etchSmoothness = smoothness;
  const covEl = getEl("etchCoverage");
  const smoothEl = getEl("etchSmoothness");
  if (covEl) covEl.innerText = Math.floor(coverage * 100).toString();
  if (smoothEl) smoothEl.innerText = Math.floor(smoothness * 100).toString();
}
...
export function setupUIEffects(): void {
  ...
  effect(() => {
    ...
    if (el) el.innerText = Math.floor(will.value).toString();
  });
  effect(() => {
    const el = getEl("healthValue");
    if (el) el.innerText = health.value.toString();
  });
  effect(() => {
    const el = getEl("nooseValue");
    const fill = getEl("nooseFill");
    if (el) el.innerText = Math.floor(kalgothsNoose.value).toString();
    if (fill) fill.style.width = kalgothsNoose.value + "%";
  });
  effect(() => {
    const el = getEl("quotaRemaining");
    if (el) el.innerText = quotaRemaining.value.toString();
  });
  effect(() => {
    const el = getEl("circlePowerValue");
    const fill = getEl("circlePowerFill");
    if (el) el.innerText = circlePower.value.toString();
    if (fill) fill.style.width = circlePower.value + "%";
  });
  effect(() => {
  ...
  effect(() => {
    ...
    if (el) el.innerText = masteryLevel.value.toString();
  });
  effect(() => {
    const el = getEl("masteryXP");
    if (el) el.innerText = masteryXP.value.toString();
  });
  effect(() => {
    const el = getEl("masteryNeeded");
    if (el) el.innerText = masteryNeeded.value.toString();
  });
  effect(() => {
    const fill = getEl("masteryFill");
    if (fill) fill.style.width = (masteryXP.value / masteryNeeded.value) * 100 + "%";
  });

  // Circle Mastery
  effect(() => {
    const el = getEl("circleMasteryValue");
    if (el) el.innerText = circleMastery.value.toString();
  });

  // Seed Resonance
  effect(() => {
    const el = getEl("seedResonanceValue") || getEl("bloodResonanceValue");
    if (el) el.innerText = `${seedResonance.value}/${maxSeedResonance.value}`;
    const consoleSeed = getEl("consoleBlood");
    if (consoleSeed) consoleSeed.innerText = `${seedResonance.value}/${maxSeedResonance.value}`;
  });

  // Familiar
  effect(() => {
    const el = getEl("familiarLevel");
    if (el) el.innerText = familiar.value.level.toString();
  });
  effect(() => {
    const fill = getEl("xpFill");
    if (fill) fill.style.width = (familiar.value.xp / familiar.value.nextXP) * 100 + "%";
  });
  effect(() => {
    const el = getEl("familiarXP");
    if (el) el.innerText = `XP: ${familiar.value.xp}/${familiar.value.nextXP}`;
  });
  effect(() => {
    const el = getEl("familiarMood");
    if (el) el.innerText = `Mood: ${familiar.value.mood}%`;
  });
  effect(() => {
    const abilities = familiar.value.abilities;
    const abilityNames: string[] = [];
    if (abilities.includes("scout")) abilityNames.push("Scout");
    if (abilities.includes("ward")) abilityNames.push("Ward");
    if (abilities.includes("guidance")) abilityNames.push("Guidance");
    if (abilities.includes("presence")) abilityNames.push("Presence");
    const el = getEl("familiarAbilities");
    if (el) el.innerHTML = abilityNames.join(" · ");
  });
  ...
  effect(() => {
    ...
    if (whispSprite) {
      ...
      if (familiar.value.abilities.includes('scout')) tip += '\nScout: +20% find';
      if (familiar.value.abilities.includes('ward')) tip += '\nWard: -15% Noose';
      if (familiar.value.abilities.includes('guidance')) tip += '\nGuidance: +20% trace';
      if (familiar.value.abilities.includes('presence')) tip += '\nPresence: +10% dominate';
  ...
  effect(() => {
    ...
    if (el) el.innerText = crafted.value.powderOfWarding.toString();
  });
  effect(() => {
    const el = getEl("consolePhial");
    if (el) el.innerText = crafted.value.phialOfSubjugation.toString();
  });
  effect(() => {
    const el = getEl("consoleIchor");
    if (el) el.innerText = (ingredients.value.demonIchor || 0).toString();
  });
  effect(() => {
    const el = getEl("consoleIntegrity");
    if (el) el.innerText = `Power: ${circlePower.value}%`;
  });
  ...
  effect(() => {
    ...
    if (escapeArea) escapeArea.style.display = canEscape ? "block" : "none";
    if (consoleEscapeArea) consoleEscapeArea.style.display = canEscape ? "block" : "none";
  });
  ...
  effect(() => {
    ...
    if (ritual) {
      ...
      if (circlePower.value >= 90) ritual.classList.add("power-full");
      else if (circlePower.value >= 60) ritual.classList.add("power-high");
      else if (circlePower.value >= 30) ritual.classList.add("power-mid");
      else if (circlePower.value > 0) ritual.classList.add("power-low");
    }
  });
  ...
  effect(() => {
    ...
    if (ashArea) ashArea.style.display = ashAvailable.value ? "block" : "none";
  });
...
export function updateUI(): void {
  initTooltips();
  let tetherCanvas = getEl("tetherCanvas") as HTMLCanvasElement | null;
  if (!tetherCanvas) {
