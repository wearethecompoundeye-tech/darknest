// js/ui/whisp-chat.ts – Zilion, Upper‑Edge Companion
// Robust DOM particle trail (comet‑like), idle bobbing near the circle,
// minigame‑aware repositioning, never crosses center.

import { el } from '../core/dom-helper.js';
import { askOllama } from '../ai/ai-engine.js';
import { gameBus } from '../core/eventBus.js';
import { GameEvents, type WhispMessagePayload } from '../core/events.js';
import {
  corruptionLevel,
  health,
  will,
  maxWill,
  isGazeActive,
} from '../core/state-signals.js';
import { getZilionMemory } from '../ai/zelionMemory.js';
import { currentPhase } from '../core/gameReducer.js';

// ── Global enable ────────────────────────────────────────────────
let whispEnabled = true;

export function setWhispEnabled(enabled: boolean): void {
  whispEnabled = enabled;
  if (S.av) S.av.style.display = enabled ? '' : 'none';
  if (S.trailParts) S.trailParts.style.display = enabled ? '' : 'none';
}
export function isWhispEnabled(): boolean { return whispEnabled; }

// ── Internal state ───────────────────────────────────────────────
const AVATAR_SIZE = 75;
const EYE_RANGE = 10;
const SPRING_STIFFNESS = 3.8;
const SPRING_DAMPING = 0.88;

const S = {
  av: null as HTMLElement | null,
  body: null as HTMLElement | null,
  eye: null as HTMLElement | null,
  pupil: null as HTMLElement | null,
  refl: null as HTMLElement | null,
  bubble: null as HTMLElement | null,
  chatM: null as HTMLDivElement | null,
  chatInp: null as HTMLInputElement | null,
  chatMsgs: null as HTMLElement | null,
  left: 0, top: 0,
  targetLeft: 0, targetTop: 0,
  velocityX: 0, velocityY: 0,
  prevLeft: 0, prevTop: 0,
  drag: false, startX: 0, startY: 0, startLeft: 0, startTop: 0,
  tX: 0, tY: 0, cX: 0, cY: 0,
  lastMouseMove: 0,
  blinkState: 1, blinkTarget: 1, nextBlink: 0,
  breathPhase: 0,
  glowIntensity: 1, glowTarget: 1, glowDecay: 0,
  emotion: 0,
  sleeping: false,
  sleepTimer: 0,
  sleepZElements: [] as HTMLElement[],
  behaviour: 'idle' as string,
  behaviourTimer: 0,
  figure8Phase: 0,
  doingFigure8: false,
  figure8StartLeft: 0, figure8StartTop: 0,
  wakingUp: false, wakeStart: 0,
  lastClick: 0,
  modalOpen: false, dodgeLeft: 0, dodgeTop: 0,
  messageQueue: [] as { sender: string; text: string }[],
  trailParts: null as HTMLElement | null,
  minigameActive: false,
};

// ── Mouse tracking ────────────────────────────────────────────────
document.addEventListener('mousemove', (e: MouseEvent) => {
  if (!whispEnabled) return;
  S.lastMouseMove = Date.now();
  if (!S.av) return;
  const r = S.av.getBoundingClientRect();
  S.tX = Math.max(-EYE_RANGE, Math.min(EYE_RANGE, (e.clientX - r.left - r.width / 2) * 0.15));
  S.tY = Math.max(-EYE_RANGE, Math.min(EYE_RANGE, (e.clientY - r.top - r.height / 2) * 0.15));
  if (S.sleeping) wakeUp();
});
window.addEventListener('resize', () => { /* no bounds recalc needed */ });

// ── Public API ────────────────────────────────────────────────────
export function initWhispChat(): void {
  if (!whispEnabled) return;
  if (S.av) return;

  // Position at the top edge of the center panel, near the circle
  const panel = document.querySelector('.center-stack');
  if (panel) {
    const rect = panel.getBoundingClientRect();
    S.left = rect.left + rect.width / 2 - AVATAR_SIZE / 2;
    S.top = rect.top + 12; // just below the top edge
  } else {
    S.left = window.innerWidth / 2 - AVATAR_SIZE / 2;
    S.top = 60;
  }
  S.targetLeft = S.left;
  S.targetTop = S.top;

  createAvatar();
  addStyles();
  setupDrag();
  startRenderLoop();
  startIdleWander();
  listenForMinigames();

  gameBus.on<WhispMessagePayload>(GameEvents.WHISP_MESSAGE, (p) => {
    addOrQueue(p.speaker === 'kalgoth' ? 'kalgoth' : 'zelion', p.text);
    getZilionMemory().recordEvent('whisp_msg', p.text);
  });

  will.subscribe(v => { if (v < 20 && !isGazeActive.value) showBubble("Low will! Use a restorative draught.", false); });
  health.subscribe(v => { if (v < 25 && !isGazeActive.value) showBubble("Health critical! Whisp cannot heal you permanently.", false); });
  isGazeActive.subscribe(v => { if (v) showBubble("Kalgoth’s Gaze! Stay in the circle!", false); });
}

export function destroyWhispChat(): void {
  if (S.cleanupInterval) clearInterval(S.cleanupInterval);
  if (S.cleanupAnimFrame) cancelAnimationFrame(S.cleanupAnimFrame);
  if (S.av) { S.av.remove(); S.av = null; }
  if (S.trailParts) { S.trailParts.remove(); S.trailParts = null; }
  if (S.bubble) { S.bubble.remove(); S.bubble = null; }
  if (S.chatM) { S.chatM.remove(); S.chatM = null; }
  S.messageQueue = [];
}

export function showBubble(text: string, isKalgoth = false): void {
  addOrQueue(isKalgoth ? 'kalgoth' : 'zelion', text);
}

function addOrQueue(sender: string, text: string): void {
  if (S.chatM) { addChatMsg(sender, text); }
  else { S.messageQueue.push({ sender, text }); }
}

// ── Edge target (upper area, never center) ────────────────────────
function computeEdgeTarget(): { left: number; top: number } {
  const panel = document.querySelector('.center-stack');
  if (!panel) return { left: window.innerWidth - AVATAR_SIZE - 20, top: 40 };
  const rect = panel.getBoundingClientRect();

  // Default: top edge, slightly right of center
  let tx = rect.left + rect.width * 0.65 - AVATAR_SIZE / 2;
  let ty = rect.top + 12;

  if (S.minigameActive) {
    // Top‑right corner
    tx = rect.right - AVATAR_SIZE - 8;
    ty = rect.top + 8;
  } else if (isGazeActive.value) {
    tx = rect.right - AVATAR_SIZE - 8;
    ty = rect.top + 8;
  }

  tx = Math.max(rect.left + 4, Math.min(rect.right - AVATAR_SIZE - 4, tx));
  ty = Math.max(rect.top + 4, Math.min(rect.bottom - AVATAR_SIZE - 4, ty));
  return { left: tx, top: ty };
}

// ── Minigame listener ─────────────────────────────────────────────
function listenForMinigames(): void {
  window.addEventListener('darknest:minigame-state', (e: CustomEvent) => {
    const { active } = e.detail || {};
    S.minigameActive = !!active;
    if (!S.drag) {
      const target = computeEdgeTarget();
      S.targetLeft = target.left;
      S.targetTop = target.top;
    }
  });
}

// ── Avatar creation (with DOM trail container) ────────────────────
function createAvatar(): void {
  const av = document.createElement('div');
  av.id = 'zelion-avatar';
  Object.assign(av.style, {
    position: 'fixed',
    left: S.left + 'px',
    top: S.top + 'px',
    width: '75px', height: '75px',
    zIndex: '100000',
    cursor: 'grab', userSelect: 'none', touchAction: 'none',
    filter: 'drop-shadow(0 6px 12px rgba(0,0,0,0.5))',
  });
  S.av = av;

  const body = document.createElement('div');
  body.className = 'zelion-body';
  Object.assign(body.style, {
    position: 'absolute', width: '100%', height: '100%',
    borderRadius: '50%',
    background: 'radial-gradient(circle at 35% 35%, #d4e8a0, #5a8a3a 60%, #2a4a1a)',
    boxShadow: 'inset 0 -8px 16px rgba(0,0,0,0.4), inset 0 4px 8px rgba(255,255,255,0.15), 0 0 20px #8ac060, 0 0 45px #4a7a2a',
    transition: 'transform 0.1s ease-out',
  });
  S.body = body;
  av.appendChild(body);

  const eye = document.createElement('div');
  Object.assign(eye.style, {
    position: 'absolute', top: '21px', left: '24px',
    width: '24px', height: '24px',
    background: 'radial-gradient(circle, #f8fff8, #d4f0a0 90%)',
    borderRadius: '50%', overflow: 'hidden',
    boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.2), 0 0 4px rgba(0,0,0,0.3)',
  });
  S.eye = eye;
  const pupil = document.createElement('div');
  Object.assign(pupil.style, {
    position: 'absolute', top: '50%', left: '50%',
    width: '12px', height: '12px', margin: '-6px 0 0 -6px',
    background: '#1a3a0a', borderRadius: '50%',
  });
  S.pupil = pupil; eye.appendChild(pupil);
  const refl = document.createElement('div');
  Object.assign(refl.style, {
    position: 'absolute', top: '50%', left: '50%',
    width: '4px', height: '4px', margin: '-8px 0 0 4px',
    background: 'white', borderRadius: '50%', opacity: 0.9,
  });
  S.refl = refl; eye.appendChild(refl);
  av.appendChild(eye);

  // Trail container (DOM)
  const trailContainer = document.createElement('div');
  trailContainer.className = 'zelion-trail';
  Object.assign(trailContainer.style, {
    position: 'fixed', top: '0', left: '0', width: '100%', height: '100%',
    pointerEvents: 'none', zIndex: '99999',
  });
  document.body.appendChild(trailContainer);
  S.trailParts = trailContainer;

  // Ambient particle container (idle sparks)
  const parts = document.createElement('div');
  parts.className = 'zelion-parts';
  Object.assign(parts.style, {
    position: 'absolute', top: '0', left: '0', width: '100%', height: '100%', pointerEvents: 'none',
  });
  av.appendChild(parts);

  document.body.appendChild(av);

  // Idle ambient particles every 400ms
  setInterval(() => {
    for (let i = 0; i < (2 + Math.floor(Math.random() * 2)); i++) {
      setTimeout(() => spawnAmbientParticle(), i * 50);
    }
  }, 400);
}

// ── Rich DOM trail particles ──────────────────────────────────────
function spawnTrailParticle(x: number, y: number, life: number = 0): void {
  if (!S.trailParts) return;
  const p = document.createElement('div');
  const size = 14 - life * 11;         // thick at head, thin at tail
  const opacity = 0.9 - life * 0.6;    // fade
  const hue = Math.random() > 0.5 ? '#ffd700' : '#a0d07a';
  Object.assign(p.style, {
    position: 'fixed',
    left: (x - size / 2) + 'px',
    top: (y - size / 2) + 'px',
    width: size + 'px',
    height: size + 'px',
    borderRadius: '50%',
    background: hue,
    opacity: opacity.toString(),
    boxShadow: `0 0 ${4 + life * 4}px ${hue}`,
    animation: 'zelionTrailFade 0.6s ease-out forwards',
    pointerEvents: 'none',
  });
  S.trailParts.appendChild(p);
  setTimeout(() => p.remove(), 600);
}

function spawnTrailSegment(fromX: number, fromY: number, toX: number, toY: number): void {
  const dist = Math.hypot(toX - fromX, toY - fromY);
  if (dist < 2) return;
  const steps = Math.floor(dist / 2);
  const stepX = (toX - fromX) / steps;
  const stepY = (toY - fromY) / steps;
  for (let i = 1; i <= steps; i++) {
    const t = i / steps;
    const x = fromX + stepX * i;
    const y = fromY + stepY * i;
    spawnTrailParticle(x, y, t);
  }
}

// ── Idle wander (eye movements) ────────────────────────────────────
let idleTimer: number | null = null;
function startIdleWander(): void {
  const loop = () => {
    if (Date.now() - S.lastMouseMove > 1500) {
      S.tX = (Math.random() - 0.5) * EYE_RANGE * 2;
      S.tY = (Math.random() - 0.5) * EYE_RANGE * 2;
    }
    idleTimer = window.setTimeout(loop, 1200 + Math.random() * 600);
  };
  idleTimer = window.setTimeout(loop, 1500);
}

// ── Drag (with trail) ─────────────────────────────────────────────
function setupDrag(): void {
  const av = S.av; if (!av) return;
  const moveHandler = (e: MouseEvent) => {
    if (!S.drag) return;
    const newLeft = S.startLeft + e.clientX - S.startX;
    const newTop = S.startTop + e.clientY - S.startY;
    // spawn trail
    const cx = S.left + AVATAR_SIZE / 2;
    const cy = S.top + AVATAR_SIZE / 2;
    const nx = newLeft + AVATAR_SIZE / 2;
    const ny = newTop + AVATAR_SIZE / 2;
    spawnTrailSegment(cx, cy, nx, ny);

    S.left = newLeft;
    S.top = newTop;
    S.targetLeft = newLeft;
    S.targetTop = newTop;
    av.style.left = S.left + 'px';
    av.style.top = S.top + 'px';
    if (S.chatM) posChatModal();
  };
  const upHandler = (e: MouseEvent) => {
    if (!S.drag) return;
    S.drag = false;
    av.style.cursor = 'grab';
    document.removeEventListener('mousemove', moveHandler);
    document.removeEventListener('mouseup', upHandler);
    const dx = e.clientX - S.startX, dy = e.clientY - S.startY;
    if (Math.sqrt(dx * dx + dy * dy) < 3) {
      const now = Date.now();
      if (now - S.lastClick < 350) { S.lastClick = 0; toggleChat(); }
      else { S.lastClick = now; }
    }
  };
  av.addEventListener('mousedown', (e: MouseEvent) => {
    if (e.button !== 0) return;
    e.preventDefault();
    S.startX = e.clientX; S.startY = e.clientY;
    S.startLeft = S.left; S.startTop = S.top;
    S.drag = true;
    av.style.cursor = 'grabbing';
    document.addEventListener('mousemove', moveHandler);
    document.addEventListener('mouseup', upHandler);
  });
}

// ── Chat modal (unchanged) ────────────────────────────────────────
function toggleChat(): void { S.chatM ? closeChat() : openChat(); }
function openChat(): void {
  if (S.chatM) return;
  const md = document.createElement('div');
  Object.assign(md.style, {
    position:'fixed', width:'340px', maxHeight:'450px',
    background:'rgba(5,2,8,0.95)', border:'2px solid #7ea04b',
    borderRadius:'20px 20px 20px 4px', boxShadow:'0 0 25px rgba(126,160,75,0.5)',
    display:'flex', flexDirection:'column', zIndex:'2147483647',
    fontFamily:'"Courier New",monospace', color:'#e0d8cc',
  });
  md.innerHTML = `<div style="padding:8px 16px;border-bottom:1px solid #5a4a3a;display:flex;justify-content:space-between;"><span style="color:#a0d07a;">🌀 Zilion</span><span style="color:#8a7a6a;font-size:0.7rem;">Esc to close</span></div>
    <div id="zelionChatMsgs" style="flex:1;overflow-y:auto;padding:10px;display:flex;flex-direction:column;gap:6px;max-height:300px;"></div>
    <div style="display:flex;border-top:1px solid #5a4a3a;"><input id="zelionChatInp" type="text" placeholder="Ask Zilion..." style="flex:1;background:transparent;border:none;padding:10px;color:#e0d8cc;font-family:inherit;outline:none;"></div>`;
  document.body.appendChild(md);
  S.chatM = md; S.chatMsgs = document.getElementById('zelionChatMsgs') as HTMLElement;
  S.chatInp = document.getElementById('zelionChatInp') as HTMLInputElement;
  S.messageQueue.forEach(m => addChatMsg(m.sender, m.text)); S.messageQueue = [];
  S.chatInp.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && S.chatInp?.value.trim()) {
      const txt = S.chatInp.value.trim(); S.chatInp.value = '';
      addChatMsg('player', txt); handleChatSend(txt);
    }
    if (e.key === 'Escape') closeChat();
  });
  posChatModal(); S.chatInp.focus();
  document.addEventListener('keydown', escClose);
}
function closeChat(): void { if (S.chatM) { S.chatM.remove(); S.chatM = null; S.chatInp = null; S.chatMsgs = null; } document.removeEventListener('keydown', escClose); }
function escClose(e: KeyboardEvent): void { if (e.key === 'Escape') closeChat(); }
function posChatModal(): void {
  if (!S.chatM || !S.av) return;
  const r = S.av.getBoundingClientRect();
  S.chatM.style.left = Math.min(r.left + r.width + 10, innerWidth - 360) + 'px';
  S.chatM.style.top  = Math.max(10, r.top) + 'px';
}
function addChatMsg(sender: string, text: string): void {
  if (!S.chatMsgs) return;
  const isPlayer = sender === 'player', isKalgoth = sender === 'kalgoth';
  const d = document.createElement('div');
  Object.assign(d.style, {
    alignSelf: isKalgoth ? 'center' : (isPlayer ? 'flex-end' : 'flex-start'),
    maxWidth:'80%', padding:'6px 10px',
    borderRadius: isPlayer ? '10px 10px 2px 10px' : (isKalgoth ? '0' : '10px 10px 10px 2px'),
    background: isKalgoth ? '#4a1a1a' : (isPlayer ? '#2a1a0a' : '#1a2a0a'),
    border:`1px solid ${isKalgoth?'#ff4444':(isPlayer?'#8a7a5a':'#5a7a3a')}`,
    fontSize:'0.8rem', fontStyle: isPlayer?'normal':'italic',
    fontWeight: isKalgoth?'bold':'normal', textShadow: isKalgoth?'0 0 5px red':'none',
  });
  d.textContent = `${isKalgoth?'KALGOTH':(isPlayer?'You':'Zilion')}: ${text}`;
  S.chatMsgs.appendChild(d); S.chatMsgs.scrollTop = S.chatMsgs.scrollHeight;
}
async function handleChatSend(userText: string): Promise<void> {
  const corr = corruptionLevel.value;
  if (corr > 50 && Math.random() < (corr - 30) / 70) { addChatMsg('kalgoth', "I hear your thoughts."); return; }
  if (corr > 80 && Math.random() < 0.2) { addChatMsg('zelion', "Kalgoth's shadow..."); return; }
  try {
    const mem = getZilionMemory(); const prompt = mem.getPersonalityPrompt();
    const reply = await askOllama([{ role: 'user', content: `${prompt}\n\nUser message: ${userText}` }]);
    addChatMsg('zelion', reply);
  } catch { addChatMsg('zelion', "*static*"); }
}

// ── Render loop ───────────────────────────────────────────────────
let animFrame: number | null = null;
let shiftPhase = 0;
function startRenderLoop(): void {
  let lastTime = performance.now();
  const loop = (now: number) => {
    const dt = Math.min(0.1, (now - lastTime) / 1000);
    lastTime = now;

    if (!S.drag) {
      const target = computeEdgeTarget();
      S.targetLeft = target.left;
      S.targetTop = target.top;

      // Spring physics
      const ax = (S.targetLeft - S.left) * SPRING_STIFFNESS;
      const ay = (S.targetTop - S.top) * SPRING_STIFFNESS;
      S.velocityX = (S.velocityX + ax * dt) * SPRING_DAMPING;
      S.velocityY = (S.velocityY + ay * dt) * SPRING_DAMPING;
      S.left += S.velocityX * dt;
      S.top += S.velocityY * dt;

      if (S.av) {
        S.av.style.left = S.left + 'px';
        S.av.style.top = S.top + 'px';
      }

      // Trail segment
      const dx = S.left - S.prevLeft, dy = S.top - S.prevTop;
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dist > 1.5) {
        spawnTrailSegment(
          S.prevLeft + AVATAR_SIZE / 2,
          S.prevTop + AVATAR_SIZE / 2,
          S.left + AVATAR_SIZE / 2,
          S.top + AVATAR_SIZE / 2
        );
      }
      S.prevLeft = S.left;
      S.prevTop = S.top;
    }

    // Eye tracking
    S.cX += (S.tX - S.cX) * 0.2;
    S.cY += (S.tY - S.cY) * 0.2;

    // Blinking
    if (now > S.nextBlink) {
      S.nextBlink = now + 2500 + Math.random() * 3000;
      S.blinkTarget = 0.05;
      setTimeout(() => { S.blinkTarget = 1; }, 120);
      if (Math.random() < 0.3) setTimeout(() => { S.blinkTarget = 0.05; setTimeout(() => { S.blinkTarget = 1; }, 80); }, 200);
    }
    S.blinkState += (S.blinkTarget - S.blinkState) * 0.5;
    S.breathPhase += 0.02;

    if (S.glowDecay > 0) { S.glowTarget += (1 - S.glowTarget) * 0.05; S.glowDecay -= 16; } else S.glowTarget = 1;
    S.glowIntensity += (S.glowTarget - S.glowIntensity) * 0.1;

    if (health.value < 20 || will.value < 15) {
      S.glowTarget = 0.6;
      S.glowDecay = 1;
    }

    // Bobbing idle
    if (!S.drag && !S.sleeping) {
      shiftPhase += 0.015;
      const sx = Math.sin(shiftPhase * 1.3) * 1.5;
      const sy = Math.cos(shiftPhase * 1.7) * 1.2;
      const scale = 1 + Math.sin(shiftPhase) * 0.005;
      S.body!.style.transform = `translate(${sx}px, ${sy}px) scale(${scale})`;
    } else if (S.sleeping) {
      S.body!.style.transform = 'scale(0.94)';
    }

    // Emotion colors
    const emoColors: [string, string][] = [
      ['#d4e8a0', '#5a8a3a'],
      ['#ffd6a5', '#ff8c37'],
      ['#c0e0a0', '#4a7a2a'],
      ['#ffffff', '#a0d07a'],
    ];
    const [c1, c2] = (health.value < 20 || will.value < 15) ? ['#ffccaa', '#dd6622'] : emoColors[S.emotion];
    if (S.body) {
      S.body.style.background = `radial-gradient(circle at 35% 35%, ${c1}, ${c2})`;
      const gs = 18 + S.glowIntensity * 20;
      const go = 0.2 + S.glowIntensity * 0.3;
      S.body.style.boxShadow = `inset 0 -8px 16px rgba(0,0,0,0.4), inset 0 4px 8px rgba(255,255,255,0.15), 0 0 ${gs}px rgba(160,208,122,${go}), 0 0 ${gs * 2}px rgba(74,122,42,${go * 0.6})`;
    }

    if (S.eye) S.eye.style.transform = `translate(${S.cX}px, ${S.cY}px) scaleY(${S.blinkState})`;
    if (S.refl) S.refl.style.transform = `translate(${S.cX * 0.3}px, ${S.cY * 0.3}px)`;

    animFrame = requestAnimationFrame(loop);
  };
  animFrame = requestAnimationFrame(loop);
}

// ── Ambient idle particles (sparks around avatar) ──────────────────
function spawnAmbientParticle(): void {
  const c = S.av?.querySelector('.zelion-parts') as HTMLElement;
  if (!c) return;
  const p = document.createElement('div');
  const a = Math.random() * Math.PI * 2, d = 25 + Math.random() * 35;
  Object.assign(p.style, {
    position: 'absolute', top: '50%', left: '50%',
    width: '4px', height: '4px',
    borderRadius: '50%',
    background: Math.random() > 0.7 ? '#ffd700' : '#a0d07a',
    boxShadow: '0 0 4px currentColor',
    animation: `zelionParticle ${1.5 + Math.random() * 2}s linear forwards`,
  });
  p.style.setProperty('--tx', `${Math.cos(a) * d}px`);
  p.style.setProperty('--ty', `${Math.sin(a) * d}px`);
  c.appendChild(p);
  setTimeout(() => p.remove(), 3000);
}

// ── Sleep / wake (unchanged) ──────────────────────────────────────
function enterSleep(): void {
  if (S.sleeping) return;
  S.sleeping = true; S.emotion = 2;
  if (S.eye) S.eye.style.transform = `translate(${S.cX}px, ${S.cY}px) scaleY(0.05)`;
  S.sleepZElements = [];
  const spawnZ = () => {
    if (!S.sleeping || !S.av) return;
    const z = document.createElement('div'); z.textContent = 'Z';
    Object.assign(z.style, {
      position: 'fixed', left: (S.left + AVATAR_SIZE / 2 - 10) + 'px', top: (S.top - 20) + 'px',
      color: '#c0e0a0', fontSize: '28px', fontFamily: '"Courier New", monospace',
      pointerEvents: 'none', textShadow: '0 0 8px #7ea04b',
      animation: 'zelionZFloat 2.5s ease-out forwards', opacity: '0.9',
    });
    document.body.appendChild(z); S.sleepZElements.push(z); setTimeout(() => z.remove(), 2500);
  };
  S.sleepTimer = window.setInterval(spawnZ, 1200);
}

function wakeUp(): void {
  if (!S.sleeping) return;
  S.sleeping = false; S.emotion = 3;
  clearInterval(S.sleepTimer);
  S.sleepZElements.forEach(z => z.remove()); S.sleepZElements = [];
  S.wakingUp = true; S.wakeStart = performance.now();
  for (let i = 0; i < 15; i++) spawnTrailParticle(S.left + AVATAR_SIZE / 2, S.top + AVATAR_SIZE / 2, 0.3);
  if (S.eye) S.eye.style.transform = `translate(${S.cX}px, ${S.cY}px) scale(1.3)`;
  clearTimeout(S.behaviourTimer); pickNewBehaviour();
}

// ── Behaviour / figure‑8 (unchanged) ──────────────────────────────
function pickNewBehaviour(): void {
  let nextDelay = 6000 + Math.random() * 8000;
  const rand = Math.random();
  if (rand < 0.15) { S.behaviour = 'figure8'; S.figure8Phase = 0; S.doingFigure8 = true; }
  else if (rand < 0.30) { S.behaviour = 'twirl'; S.doingFigure8 = false; }
  else if (rand < 0.55) { S.behaviour = 'drift'; S.doingFigure8 = false; }
  else if (rand < 0.70) { S.behaviour = 'shiver'; S.doingFigure8 = false; }
  else { S.behaviour = 'idle'; S.doingFigure8 = false; }
  S.behaviourTimer = window.setTimeout(pickNewBehaviour, nextDelay);
}
pickNewBehaviour();

// ── Styles ────────────────────────────────────────────────────────
function addStyles(): void {
  if (document.getElementById('zelion-final-styles')) return;
  const s = document.createElement('style');
  s.id = 'zelion-final-styles';
  s.textContent = `
    @keyframes zelionParticle { 0% { transform: translate(0,0) scale(0); opacity:1; } 100% { transform: translate(var(--tx),var(--ty)) scale(1.5); opacity:0; } }
    @keyframes zelionTrailFade { 0% { opacity:0.9; transform: scale(1); } 100% { opacity:0; transform: scale(0.1); } }
    @keyframes zelionZFloat { 0% { opacity:1; transform:translate(0,0); } 100% { opacity:0; transform:translate(-10px,-40px); } }
  `;
  document.head.appendChild(s);
}