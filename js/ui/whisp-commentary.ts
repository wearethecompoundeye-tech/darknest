...
let commentaryTimeout: number | null = null;
...
export async function whispSpeak(context: string, stateSummary: string): Promise<void> {
  ...
  commentaryTimeout = window.setTimeout(() => {
    if (bubble) {
      ...
      setTimeout(() => { if (bubble) bubble.style.display = 'none'; }, 500);
    }
  }, 5000);
}

export function whispSay(message: string): void {
  if (!whispEnabled) return;
  if (!bubble) bubble = createBubble();
  bubble.textContent = message;
  bubble.style.display = 'block';
...
export function setupWhispReactions(): void {
  // Health changes
  effect(() => {
  ...
  effect(() => {
    ...
    if (currentComboCount > lastComboCount && currentComboCount > 0) {
      ...
      const cardNames = newCombo.cardIds.map(id => {
        const card = ownedCards.value.find((c: any) => c.cardId === id);
        return card?.name || id;
      }).join(' and ');
